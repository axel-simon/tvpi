/* Test program to check widening. Based on testPolyhedron<true>Hull6.cpp.
 */
...
int main(int argc) {
  ...
  bool p3p1W = p3.includes(xBound1, yBound1, p1);
  if (chat) cerr << "widened includes p1 : " << (p3p1W ? "yes" : "no") <<endl;
  bool p3p2W = p3.includes(xBound2, yBound2, p2);
  if (chat) cerr << "widened includes p2 : " << (p3p2W ? "yes" : "no") << endl;

  bool correct= p3p1 && p3p2 && p3p1W && p3p2W;
